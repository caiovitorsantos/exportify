...
class ConfigTest < Minitest::Test
  def setup
    @original_path = Exportify::Config::CONFIG_PATH
    @tmp_config    = File.join(Dir.mktmpdir, '.exportify')
    Exportify::Config.stub_const(:CONFIG_PATH, @tmp_config) {} rescue nil
  end

  def with_tmp_config
    Exportify::Config.stub(:load, -> { File.exist?(@tmp_config) ? JSON.parse(File.read(@tmp_config)) : {} }) do
      Exportify::Config.stub(:save, ->(data) { File.write(@tmp_config, JSON.pretty_generate(data)) }) do
        yield
      end
    end
  end
